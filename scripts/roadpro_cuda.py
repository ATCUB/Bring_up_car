...
def detect_intersection_type():
    ...
    while(True):
        ...
        gray_gpu = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
            # 使用阈值化处理，使黑色循迹线变成白色，背景变成黑色
        _, thresh = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY_INV)
        _, thresh_gpu = cv2_cuda.threshold(cv2_cuda_GpuMat(gray_gpu), 100, 255, cv2.THRESH_BINARY_INV)

            # 使用膨胀操作去除噪声
        kernel = np.ones((5, 5), np.uint8)
        dilated = cv2.erode(thresh, kernel, iterations=1)
        dilated_gpu = cv2_cuda.erode(cv2_cuda_GpuMat(thresh_gpu), kernel, iterations=1)
        dilated_1 = cv2.erode(dilated, kernel, iterations=1)
        dilated_1_gpu = cv2_cuda.erode(cv2_cuda_GpuMat(dilated_gpu), kernel, iterations=1)
        dilated = cv2.dilate(dilated_1, kernel, iterations=1)
        dilated_gpu = cv2_cuda.dilate(cv2_cuda_GpuMat(dilated_1_gpu), kernel, iterations=1)

        cv2.imshow("dilated",dilated)

        # 使用霍夫变换查找直线
        lines = cv2.HoughLinesP(dilated, 1, np.pi / 180, 100, minLineLength=100, maxLineGap=100)
        lines_gpu = cv2_cuda.HoughLinesP(cv2_cuda_GpuMat(dilated_gpu), 1, np.pi / 180, 100, minLineLength=100, maxLineGap=100)
